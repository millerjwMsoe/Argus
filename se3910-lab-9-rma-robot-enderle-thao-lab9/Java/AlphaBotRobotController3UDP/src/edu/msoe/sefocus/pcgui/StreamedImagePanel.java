...
import edu.msoe.sefocus.core.LongListWithStatistics;
...
public class StreamedImagePanel extends JPanel implements Runnable {
	...
	public void run() {
		...
		CapturedImage imageBeingReceived = new CapturedImage(this.imageHeight, this.imageWidth, Integer.MIN_VALUE, 0);

		byte[] receptionBuffer = new byte[this.imageWidth * 3 + 24];
		byte[] lineRGBDataBuffer = new byte[this.imageWidth * 3];

		while (keepGoing) {
			// Check that the buffer sizes are still correct. If something has changed,
			// modify the sizes again.
			if ((receptionBuffer.length != this.imageWidth * 3 + 24)
					|| (lineRGBDataBuffer.length != this.imageWidth * 3)) {
				receptionBuffer = new byte[this.imageWidth * 3 + 24];
				lineRGBDataBuffer = new byte[this.imageWidth * 3];
			...
			try {
				...
				dis.readFully(lineRGBDataBuffer, 0, 3 * imageWidth);

				if (ccol != imageWidth || crow != imageHeight) {
					changeSize(ccol, crow);
				}

				if (cimageNumber == imageBeingReceived.id) {
					// Add the data in.
					...
					// Handle a new image...
					imageBeingReceived = new CapturedImage(this.imageHeight, this.imageWidth, cimageNumber, startTime );
					imageBeingReceived.addLine(lineRGBDataBuffer, cseg);
					receptionBuffer = new byte[this.imageWidth * 3 + 24];
					lineRGBDataBuffer = new byte[this.imageWidth * 3];
				}
				if (imageBeingReceived.done) {
					displayImage(imageBeingReceived);
					imageBeingReceived = new CapturedImage(this.imageHeight, this.imageWidth, cimageNumber, startTime );
					receptionBuffer = new byte[this.imageWidth * 3 + 24];
					lineRGBDataBuffer = new byte[this.imageWidth * 3];
				...
				imageBeingReceived = new CapturedImage(this.imageHeight, this.imageWidth, 0, 0);
				receptionBuffer = new byte[this.imageWidth * 3 + 24];
				lineRGBDataBuffer = new byte[this.imageWidth * 3];
	...
	public void displayImage(CapturedImage receivedImage) {
		...
		DataBuffer buffer = new DataBufferByte(receivedImage.myArray, receivedImage.myArray.length);

		// The most difficult part of awt api for me to learn
		SampleModel sampleModel = new ComponentSampleModel(DataBuffer.TYPE_BYTE, imageWidth, imageHeight, 3,
				imageWidth * 3, new int[] { 2, 1, 0 });
		Raster raster = Raster.createRaster(sampleModel, buffer, null);

		// Create a new image and assign the photo to it.
		BufferedImage localImage = new BufferedImage(imageWidth, imageHeight, BufferedImage.TYPE_3BYTE_BGR);
		localImage.setData(raster);
		localImage = toBufferedImage(rotateImage(localImage, 0));
		final ImageIcon imageIconToUse = new ImageIcon(localImage);
		...
		if (logStream != null) {
			logStream.println("Frame: " + receivedPhotoCount + "\tLines Received: " + receivedImage.getLinesReceived() + "\tHeight: " + imageHeight + "\tWidth: " + imageWidth
	 ...
	 * 
	 * @param img    This is the image.
	 * @param degree This is the angle in Radians.
	 * @return A rotated image will be returned.
	 */
